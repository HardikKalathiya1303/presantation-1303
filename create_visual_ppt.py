...
def add_image_slide(prs, title, image_path, caption=None):
    ...
    if os.path.exists(image_path):
        # Add image
        slide.shapes.add_picture(image_path, Inches(1.5), Inches(1.8), 
                                width=Inches(7), height=Inches(4))
        
        # Add caption if provided
        if caption:
            caption_shape = slide.shapes.add_textbox(
                Inches(1.5), Inches(5.8), Inches(7), Inches(0.5))
            text_frame = caption_shape.text_frame
            p = text_frame.add_paragraph()
            p.text = caption
            p.alignment = PP_ALIGN.CENTER
        ...
        # Add error message if image doesn't exist
        error_shape = slide.shapes.add_textbox(
            Inches(1.5), Inches(3), Inches(7), Inches(1))
        text_frame = error_shape.text_frame
        p = text_frame.add_paragraph()
        p.text = f"Image not found: {image_path}"
        p.alignment = PP_ALIGN.CENTER
        p.runs[0].font.italic = True
        p.runs[0].font.size = Pt(14)
        p.runs[0].font.color.rgb = RGBColor(231, 76, 60)  # Red color for error
...
def create_visual_presentation():
    ...
    # 5. Features Visual Slide
    add_image_slide(prs, "Key Features Visualization", 
                   "images/feature_chart.svg", 
                   "Visual representation of ShopSleek's integrated features")
    
    # 6. Features Two Column
    left_features = [
    ...
    add_image_slide(prs, "Three-Tier Architecture", 
                   "images/architecture.svg", 
                   "ShopSleek's Three-Tier Architecture: Presentation, Application, and Data Layers")
    ...
    # 10. Test Cases Section
    add_section_slide(prs, "Test Cases")
    
    # 11. Test Results Chart
    add_image_slide(prs, "Test Results Summary", 
                   "images/test_case_chart.svg", 
                   "Overall test success rate: 88.9% across all functional areas")
    
    # 12. User Authentication Test Cases
    auth_test_cases = [
    ...
    # 16. Security Diagram
    add_image_slide(prs, "Security Architecture", 
                   "images/security_diagram.svg", 
                   "Multi-layered security architecture protecting user data and transactions")
    ...
    add_image_slide(prs, "Deployment Infrastructure", 
                   "images/deployment_diagram.svg", 
                   "ShopSleek's cloud deployment architecture on HostingRaja infrastructure")
    
    # 20. Deployment Details Two Column
    left_deployment = [
